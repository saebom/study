...
# x_train = torch.FloatTensor(x_train) # 에러 ValueError: could not determine the shape of object type 'DataFrame'
# x_train = torch.FloatTensor(x_train.values) # values와 동일. dataframe을 numpy로 변환
x_train = torch.FloatTensor(x_train.to_numpy()).to(DEVICE) # dataframe을 numpy로 변환
# y_train = torch.FloatTensor(y_train.values).unsqueeze(1).to(DEVICE)   # 에러 AttributeError: 'Series' object has no attribute 'valuse'
y_train = torch.FloatTensor(y_train.to_numpy()).unsqueeze(1).to(DEVICE)
x_test = torch.FloatTensor(x_test.to_numpy()).to(DEVICE)
# y_test = torch.FloatTensor(y_test.valuse).unsqueeze(-1).to(DEVICE)    # 에러 AttributeError: 'Series' object has no attribute 'valuse'
