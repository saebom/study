# 하이퍼 파라미터 노드 추가, learning_rate 추가
...
def build_model(drop=0.5, optimizer='adam', activation='relu', 
                node1=64, node2=128, node3=256, lr=0.001):
    ...
    x = Dropout(drop)(x)
    x = Dense(node2, activation=activation, name='hidden3')(x)
    x = Dropout(drop)(x)
    x = Dense(node3, activation=activation, name='hidden4')(x)
    x = Dropout(drop)(x)
    x = Dense(node2, activation=activation, name='hidden5')(x)
    x = Dropout(drop)(x)
